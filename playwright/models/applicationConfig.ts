  ...
  const self = {
    clone: () => {
      ...
      Object.entries(scripts).forEach(([k, v]) =>
        clone.addScript(k as keyof typeof scripts, v),
      )
    ...
    setEnvFormatter: (
      type: keyof typeof envFormatters,
      formatter: typeof envFormatters.public,
    ) => {
    ...
    addScript: (name: keyof typeof scripts, cmd: string) => {
      if (!Object.keys(scripts).includes(name)) {
        throw new Error(
          `Invalid script name: ${name}, must be one of ${Object.keys(
            scripts,
          ).join(", ")}`,
        )
      }
    ...
    commit: async (opts?: { stableHash?: string }) => {
      ...
      for (const template of templates) {
        logger.info(
          `Copying template "${path.basename(template)}" -> ${appDirPath}`,
        )
        await fs.ensureDir(appDirPath)
        await fs.copy(template, appDirPath, {
          overwrite: true,
